...
public final class GlWorkerLoop {
	...
	private static GlWorkerLoop instance = null;
	private static CountDownLatch instanceLatch = new CountDownLatch(1);

	/**
	 * Initializes the worker loop.
	 *
	 * This method is synchronized to make sure the calling thread's local cache gets flushed.
	 */
	public synchronized static void initialize() {
		instance = new GlWorkerLoop();
		instanceLatch.countDown();
	}

	/**
	 * Blocks the calling thread until the OpenGL worker loop has been initialized.
	 */
	 ...
	 * @return the instance
	 */
	public static GlWorkerLoop getInstance() {
		return instance;
	}
	...
	public void workUntilEmpty() {
		while (true) {
			final GlWorkUnit workUnit = queue.poll();
	...
	public void workAndWait() throws InterruptedException {
		while (true) {
			final GlWorkUnit workUnit = queue.take();
	 ...
	 */
	public void execute(final GlWorkUnit workUnit) {
	 ...
	 */
	public void schedule(final GlWorkUnit workUnit) {
		queue.add(workUnit);
	}
	 ...
	 */
	public void setFrameSkipThreshold(final int frameSkipThreshold) {
	 ...
	 */
	public void setOverloadThreshold(final int overloadThreshold) {
	 ...
	 * Used for "marker" WUs.
	 */
	private static final class NullWorkUnit implements GlWorkUnit {
